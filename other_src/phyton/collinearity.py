...
csv= pd.read_csv('.local/outputs/FINAL_RFS/KARATAU-OLD/datasets/presence.csv')
...
X=list(df.drop(['id',"longitude","latitude"], axis=1).columns)
...
plt.savefig('.local/outputs/FINAL_RFS/KARATAU-OLD/datasets2/collinearity.png')
...
model = sm.OLS(df["air_density_50"],df[X])
results = model.fit()
print(results)
vif=vif(df, df[X])
print(vif)
vif.to_csv('.local/outputs/FINAL_RFS/KARATAU-OLD/datasets2/VIF.csv',sep=",")
